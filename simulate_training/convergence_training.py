...
"""
Run simulated fault training without swapping of layers
"""
rank = int(argv[3])
...
if config["dataset"] == "OpenWebText":
    ...
    ds = RedPyjamav2(tokenizer,batch_size=batch_size, seq_l=seq_l,name="default",skip=0*(world_data_size*mb_count) + validation_amount*2)
...
last_failure = 0
checkpoints_first = None
checkpoints_last = None
for itr in range(start_iter,max_iterations):
    try:
        ...
        for optim in optimizers:
            ...
            optim.step(itr)
        if itr % 100 == 0:
            checkpoints_first = deepcopy(stages[1].state_dict())
            checkpoints_last = deepcopy(stages[-1].state_dict())
